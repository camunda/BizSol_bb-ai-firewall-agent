 ...
 *
 * <p>Uses {@code @CamundaSpringProcessTest} with {@code @SpringBootTest} which is wired to a local
 * c8run instance (see application.yaml). {@link CamundaClient} and {@link
 * CamundaProcessTestContext} are injected via {@code @Autowired}.
 *
 * <p>The "safeguard prompt" task is a Camunda Connector (job type {@code
 * io.camunda.agenticai:aiagent:1}). Since the Connectors Runtime is not running in the test, the
 * {@code resultVariable}/{@code resultExpression} task headers are not evaluated. We mock the
 * connector with {@code processTestContext.mockJobWorker()} which completes the job and sets
 * variables directly on the process scope – exactly what the downstream flow expects.
 *
 * <p>The "json-converter" task is handled by the Spring {@code @JobWorker} bean {@link
 * JsonConverterWorker}. We disable it via the property {@code
 * camunda.client.worker.defaults.enabled=false} and mock it as well so that the test controls all
 * variable values.
 */
@SpringBootTest(
        properties = {
...
public class SafeguardAgentProcessTest {
    ...
    void setUp() {
        ...
        client.newDeployResourceCommand().addResourceFromClasspath(BPMN_RESOURCE).send().join();
    }
    ...
    void shouldCompleteHappyPath() throws Exception {
        ...
        for (var job : allJobs.items()) {
            log.info(
                    "  Job: type={} state={} kind={} retries={} key={} elementId={} tenantId={} worker={}",
                    job.getType(),
                    job.getState(),
                    job.getKind(),
                    job.getRetries(),
        ...
        var filteredJobs =
                client.newJobSearchRequest()
                        .filter(
                                f ->
                                        f.type(JOB_TYPE_AI_AGENT)
                                                .state(
                                                        s ->
                                                                s.in(
                                                                        io.camunda.client.api.search
                                                                                .enums.JobState
                                                                                .CREATED,
                                                                        io.camunda.client.api.search
                                                                                .enums.JobState
                        ...
                        .join();
        log.info(
                "=== FILTERED JOBS (type={}, state IN (CREATED,FAILED,RETRIES_UPDATED,TIMED_OUT), retries>=1) ===",
                JOB_TYPE_AI_AGENT);
        log.info("  Count: {}", filteredJobs.items().size());
        for (var job : filteredJobs.items()) {
            log.info(
                    "  Job: type={} state={} kind={} retries={} key={}",
                    job.getType(),
                    job.getState(),
                    job.getKind(),
                    job.getRetries(),
    ...
    /**
     * When the AI agent returns a response whose JSON does not contain the required "decision" and
     * "confidence" fields, the process should throw an escalation via the "bad agent output" path.
     */
    @Test
    void shouldEscalateOnBadAgentOutput() {
        // given – AI agent returns something, but JSON converter returns incomplete data
        processTestContext
                .mockJobWorker(JOB_TYPE_AI_AGENT)
    ...
    /**
     * When confidence is below the minimum threshold, the process should loop back and retry. After
     * exceeding maxTries the process escalates.
     */
    @Test
    void shouldRetryOnLowConfidenceAndEscalateOnMaxIterations() {
        ...
        assertThatProcessInstance(processInstance)
                .hasCompletedElements(
                        byName("max iterations reached?"), byName("safeguard prompt"));
    }
}
