...
/**
 * Process test for <em>safeguard-agent</em> that mocks the underlying LLM
 * layer using WireMock — following the same principle as the Camunda
 * connectors e2e tests in {@code camunda/connectors}.
 *
 * <h3>How it works</h3>
 * <ol>
 * <li>The BPMN is deployed <strong>unmodified</strong> (connector template
 * markers stay intact).</li>
 * <li>c8run's embedded Connectors Runtime picks up the AI-agent job
 * (type {@code io.camunda.agenticai:aiagent:1}) and calls the
 * OpenAI-compatible endpoint configured in the BPMN input mappings:
 * {@code http://localhost:11434/v1}.</li>
 * <li><strong>WireMock</strong> listens on port 11434 and returns a canned
 * chat-completion response – this is our "LLM mock".</li>
 * <li>The Connectors handler processes the response, evaluates
 * {@code resultExpression}, and completes the job with the extracted
 * variables ({@code responseJSONString}, {@code execTime}, …).</li>
 * <li>The {@code json-converter} service task is a plain Zeebe job
 * (no connector markers) and is mocked via
 * {@link CamundaProcessTestContext#mockJobWorker}.</li>
 * </ol>
 *
 * <h3>Prerequisites</h3>
 * <ul>
 * <li>c8run must be running (the test uses {@code runtime-mode: remote}).</li>
 * <li><strong>Ollama must NOT be running</strong> on port 11434, because
 * WireMock needs to bind to that port.</li>
 * </ul>
 */
@SpringBootTest(classes = SafeguardAgentProcessTest1.TestProcessApplication.class, properties = "camunda.client.worker.defaults.enabled=false")
@CamundaSpringProcessTest
class SafeguardAgentProcessTest1 {

  /**
   * Minimal Spring Boot app that deploys all BPMN, DMN & Form resources from the
   * classpath.
   */
  ...
  static void startLlmMock() {
    llmServer = new WireMockServer(
        WireMockConfiguration.wireMockConfig().port(LLM_PORT));
    llmServer.start();
  ...
  /**
   * Builds an OpenAI-compatible chat-completion response whose
   * {@code content} is a safeguard JSON verdict with decision "allow".
   */
  private static String safeChatCompletionResponse() {
    String safeguardJson = "{\"decision\":\"allow\","
        + "\"risk_labels\":[],"
  ...
  /**
   * Wraps the given assistant text in a minimal OpenAI-compatible
   * {@code /v1/chat/completions} JSON response.
   */
